...
public class CreateMapTestScreen implements Screen { // ,InputProcessor {
	...
	public void render(float delta) {
		 ...
		 * game.shapeRenderer.begin(ShapeType.Filled); int tileSize = 40;
		 * for(int i = 0; i < mainMap.mapSize; i ++){ for(int j = 0; j <
		 * mainMap.mapSize; j++){ switch(mainMap.map[i][j].fieldCategory){ case
		 * EMPTY: game.shapeRenderer.setColor(new Color(0, .5f, 0, 1)); break;
		 * case BUILDING: float color =
		 * 1-((1.f/255.f)*((4.f*mainMap.map[i][j].blockID+1.f))%255);
		 * game.shapeRenderer.setColor(new Color(color, 0, 0, 1)); break; case
		 * STREET: game.shapeRenderer.setColor(Color.DARK_GRAY); break; }
