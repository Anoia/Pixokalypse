...
public class ParseTextureAtlasTestScreen implements Screen { 
	...
	public void render(float delta) {
		...
		if (addNewActors) {
			...
			for (int i = 0; i < 10; i++) {
				actorList.add(new TestActor(width, height, spriteContainer
						.getRandomSpriteName()));
			}
			if (Gdx.graphics.getFramesPerSecond() < 45
					&& Gdx.graphics.getFramesPerSecond() > 10)
				countdownTillStop--;
			if (countdownTillStop < 1)
				addNewActors = false;
		}
		...
		while (itr.hasNext()) {
			...
			if (testActor.isOutOfWindow())
				testActor.resertActor();
			else
				testActor.updatePosition(delta);
		}
		...
		game.batch.begin();
		if (!addNewActors)
			game.font24.draw(game.batch, "Anz gerenderte Tiles pro Frame: "
					+ actorList.size(), 20, Gdx.graphics.getHeight() / 2);
		game.font24.draw(game.batch,
				"fps: " + Gdx.graphics.getFramesPerSecond(), 30, 30);
		game.font12.draw(game.batch, "(" + countdownTillStop
				+ ")Anz gerenderte Tiles pro Frame: " + actorList.size(),
				Gdx.graphics.getWidth() / 2, 13);
		game.batch.end();
	}
