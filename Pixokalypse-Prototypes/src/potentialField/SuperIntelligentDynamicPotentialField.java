...
public class SuperIntelligentDynamicPotentialField extends
		DynamicPotentialField {
	...
	public SuperIntelligentDynamicPotentialField(
			StaticPotentialField environmentMap, int distance, Boolean pushing,
			int x, int y) {
	...
	private HashSet<GridPoint2> getEmptyNeighbours(HashSet<GridPoint2> points) {
		...
		for (GridPoint2 point : points) {
			if (point.x + this.x > 0
					...
					point.x > 0 && point.x < this.width - 1 && point.y > 0
					&& point.y < this.height - 1) {
				// oben
				if (fieldArray[point.x][point.y - 1] == -1
						&& environmentMap.fieldArray[point.x + this.x][point.y
								- 1 + this.y] == 0) {
					neighbours.add(new GridPoint2(point.x, point.y - 1));
				}
				// oben-Rechts
				// if(potentialFieldMap[point.x+1][point.y-1] == -1 &&
				// environmentMap.potentialFieldMap[point.x+1+this.x][point.y-1+this.y]
				// == 0){
				// neighbours.add(new GridPoint2(point.x+1, point.y-1));
				// }
				// rechts
				if (fieldArray[point.x + 1][point.y] == -1
						&& environmentMap.fieldArray[point.x + 1 + this.x][point.y
								+ this.y] == 0) {
	...
	// private HashSet<GridPoint2> getEmptyNeighbours(HashSet<GridPoint2>
	// points){
	// HashSet<GridPoint2> neighbours = new HashSet<GridPoint2>();
	// for(GridPoint2 point: points){
	...
	/*
	 * public static void main(String[] arg){ CircleDynamicPotentialField c =
	 * new CircleDynamicPotentialField(9, false); c.printASCII(); }
	 */
